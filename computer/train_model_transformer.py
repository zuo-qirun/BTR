"""
电池热失控预测 - Transformer 模型
使用 Transformer 编码器自动提取时间序列特征
"""
...
# ====== 配置参数 ======
DATA_DIR = "data"                          # 数据目录
MODEL_DIR = "models_transformer"            # 模型保存目录
...
TRANSFORMER_HEAD_SIZE = 64                     # 每个头的维度
TRANSFORMER_FF_DIM = 128                        # 前馈网络维度
TRANSFORMER_BLOCKS = 2                          # Transformer 块数量
DROPOUT = 0.1                                   # Dropout 比率

class BatteryThermalTransformer:
    """基于 Transformer 的热失控预测器"""
    ...
    def build_transformer_model(self, input_shape):
        ...
        # 初始投影到 HEAD_SIZE 维度
        x = layers.Dense(TRANSFORMER_HEAD_SIZE)(inputs)

        for _ in range(TRANSFORMER_BLOCKS):
            # 多头自注意力
            attn_out = layers.MultiHeadAttention(
                num_heads=TRANSFORMER_HEADS,
                key_dim=TRANSFORMER_HEAD_SIZE,
                dropout=DROPOUT
            )(x, x)
            ...
            # 前馈网络
            ffn = layers.Dense(TRANSFORMER_FF_DIM, activation='relu')(x)
            ffn = layers.Dense(TRANSFORMER_HEAD_SIZE)(ffn)   # 关键修改：输出维度改为 HEAD_SIZE
    ...
    def evaluate(self, X_test, y_test):
        ...
        # 默认阈值结果
        print("\n" + "="*60)
        print("Evaluation Report - Transformer")
        print("="*60)
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            f.write(f"Evaluation Report - Transformer\n")
    ...
    def save_model(self):
        """保存模型、标准化器和配置"""
        os.makedirs(MODEL_DIR, exist_ok=True)
        self.model.save(os.path.join(MODEL_DIR, 'transformer_model.h5'))
    ...
    def load_model(self, model_dir=MODEL_DIR):
        ...
        self.model = keras.models.load_model(os.path.join(model_dir, 'transformer_model.h5'))
...
def main():
    print("="*60)
    print("电池热失控预测 - Transformer 模型")
    print("="*60)

    # 初始化
    predictor = BatteryThermalTransformer()
    ...
    predictor.model = predictor.build_transformer_model(input_shape)
